...
function buildSection(ctx, fragment, topMarginForOuterGap, toolTipTextStyle) {
    ...
    each(subBlocks, function (subBlock, idx) {
        ...
        valueFormatter ? extend(extend({}, ctx), { valueFormatter: valueFormatter }) : ctx, subBlock, idx > 0 ? gaps.html : 0, toolTipTextStyle);
...
function buildNameValue(ctx, fragment, topMarginForOuterGap, toolTipTextStyle) {
    ...
    var valueFormatter = fragment.valueFormatter || ctx.valueFormatter || (function (value) {
        ...
        return map(value, function (val, idx) { return makeValueReadable(val, isArray(valueTypeOption) ? valueTypeOption[idx] : valueTypeOption, useUTC); });
    });
    if (noName && noValue) {
        return;
    }
