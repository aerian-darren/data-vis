...
import { extend, indexOf, isArrayLike, isObject, keys, isArray, each, isString, isGradientObject, map } from 'zrender/src/core/util';
...
export function findComponentHighDownDispatchers(componentMainType, componentIndex, name, api) {
    ...
    return { focusSelf: focusSelf, dispatchers: dispatchers };
}
export function handleGlobalMouseOverForHighDown(dispatcher, e, api) {
    ...
    if (dispatchers) {
        ...
        each(dispatchers, function (dispatcher) { return enterEmphasisWhenMouseOver(dispatcher, e); });
    }
    else {
...
export function handleGlobalMouseOutForHighDown(dispatcher, e, api) {
    ...
    if (dispatchers) {
        each(dispatchers, function (dispatcher) { return leaveEmphasisWhenMouseOut(dispatcher, e); });
    }
    else {
